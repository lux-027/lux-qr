...
export default function ShareButton({ variant = 'outline', className = '' }: ShareButtonProps) {
  ...
  const shareOptions = [
    ...
    },
    {
      ...
      logo: (
        <svg viewBox="0 0 24 24" className="w-6 h-6 md:w-7 md:h-7" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    <>
      ...
      >
        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <circle cx="18" cy="5" r="3" stroke={`url(#${gradientId})`} strokeWidth="1.5" />
          <circle cx="6" cy="12" r="3" stroke={`url(#${gradientId})`} strokeWidth="1.5" />
          <circle cx="18" cy="19" r="3" stroke={`url(#${gradientId})`} strokeWidth="1.5" />
          <line x1="8.59" y1="13.51" x2="15.42" y2="17.49" stroke={`url(#${gradientId})`} strokeWidth="1.5" strokeLinecap="round" />
          <line x1="15.41" y1="6.51" x2="8.59" y2="10.49" stroke={`url(#${gradientId})`} strokeWidth="1.5" strokeLinecap="round" />
        </svg>
      </button>
      ...
      {mounted && open && createPortal(
        ...
        >
          {/* Modal card */}
          <div className="relative w-[92%] max-w-[320px] md:max-w-[420px] rounded-3xl overflow-hidden"
            style={{
              background: '#ffffff',
            ...
            {/* ── App header ── */}
            <div className="flex items-center gap-3 px-4 pt-4 pb-3 sm:px-5 sm:pt-5 sm:pb-4 md:px-6 md:pt-6 md:pb-5"
              style={{ background: 'linear-gradient(135deg,#4f46e5 0%,#7c3aed 60%,#0891b2 100%)' }}>
              {/* close X */}
              <button onClick={() => setOpen(false)}
                className="absolute top-3 right-3 sm:top-4 sm:right-4 md:top-5 md:right-5 w-7 h-7 rounded-full flex items-center justify-center"
              ...
              <div className="min-w-0">
                ...
                <p className="text-white/70 text-[11px] sm:text-xs md:text-sm font-medium leading-tight">Ücretsiz QR Kod Platformu</p>
                <p className="text-white/50 text-[10px] font-mono mt-0.5 truncate">{SITE_URL}</p>
              </div>
            </div>

            {/* ── PAYLAŞ label ── */}
            <div className="pt-4 pb-2 px-4 sm:pt-5 sm:pb-3 sm:px-5 md:pt-6 md:pb-4 md:px-6">
              <p className="text-[10px] sm:text-[11px] md:text-xs font-black uppercase tracking-[0.16em] text-gray-400 text-center">Paylaş</p>
            </div>

            {/* ── Share icon grid ── */}
            <div className="grid grid-cols-4 gap-2 md:gap-3 px-4 pb-4 sm:px-5 sm:pb-5 md:px-6 md:pb-6">
              {shareOptions.map(({ label, bg, logo, href }) => (
                href ? (
                  <a key={label} href={href} target="_blank" rel="noopener noreferrer"
                    className="flex flex-col items-center gap-2 group">
                    <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-2xl flex items-center justify-center shadow-md transition-transform duration-150 group-hover:scale-105"
                      style={{ background: bg }}>
                      {logo}
                    </div>
                    <span className="text-[10px] sm:text-[11px] md:text-xs text-gray-600 font-medium">{label}</span>
                  </a>
                ) : (
                  <button key={label}
                    onClick={async () => {
            ...
            {/* ── URL copy row ── */}
            <div className="mx-4 mb-3 sm:mx-5 sm:mb-4 md:mx-6 md:mb-5 flex items-center gap-2 px-3 py-2.5 md:px-4 md:py-3 rounded-xl"
              style={{ background: '#f3f4f6', border: '1px solid #e5e7eb' }}>
              <Link2 className="w-3.5 h-3.5 md:w-4 md:h-4 text-gray-400 flex-shrink-0" />
              <span className="text-xs md:text-sm text-gray-500 font-mono flex-1 truncate">{SITE_URL}</span>
              <button onClick={copyLink}
                ...
                {copied
                  ...
                  : <Copy className="w-4 h-4 md:w-5 md:h-5 text-indigo-400" />}
              </button>
            </div>
